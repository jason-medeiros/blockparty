...
// Default search path for yahoo, change this if yahoo changes.  Format characters are substituted at run.
const char * bp_yahoo_default_search = "http://search.yahoo.com/search?p=%s&ei=UTF-8&xargs=0&pstart=1&b=%d";

// this indicates a successful page retrieval
const char * yahoo_completion_indicator = "Searches related to <em>";

// no more results (secondary request completion indicator)
const char * yahoo_completion_indicator_2 = ">Next<";

// captcha flagged indicator
const char * yahoo_captcha_indicator_1 = "sorry.yahoo.com/sorry";
...
P_BP_YAHOOSCRAPE_SESSION BP_OpenNewYahooSearch(char *search_query, P_BP_HTTP_SESSION http_session)
{
	...
	P_BP_YAHOOSCRAPE_SESSION session = (P_BP_YAHOOSCRAPE_SESSION) bpcalloc(sizeof(BP_YAHOOSCRAPE_SESSION), 1);

	// create new garbage collector
	session->gc = BP_CreateNewGarbageCollector("yahooscrape collector", 1);

	// --- Fill in Structure Values -------------------

	// set the web request pointer in our yahooscrape session
	session->http_session = http_session;

	// duplicate the default search  search string (used as a basis for a sprintf replace later)
	session->yahoo_default_search = bpstrdup( (char *) bp_yahoo_default_search);
...
BP_ERROR_T BP_RetrieveYahooResult(P_BP_YAHOOSCRAPE_SESSION session, size_t seconds_between_request)
{
	...
	{
		printf("\n [!!] Yahoo search did not complete successfully");
		return ERR_FAILURE;
	}
	...
	size_builder += bpstrlen(session->yahoo_default_search);
	...
	printf("\n encoded query: %s", encoded_query);

	// build the new request url
	snprintf(search_request, size_builder-1, session->yahoo_default_search, encoded_query, session->start_at_result_number_xx);



	// --------- Run Request -------------------------------

	size_t yahoo_retries = 0;

	repeat_request:
	while(BP_HTTPRequest(session->http_session, search_request) != ERR_SUCCESS)
	{
		printf("\n [!!] YahooSubsystem: Could not make search request for (retrying w/ new proxy if availabile) %s", search_request);
		sleep(3);
	}

	// a bad yahoo request is < 1k in size consistently (legit requests are > 100000 normally)
	if(session->http_session->page->page_data_size < 1000)
	{
		printf("\n [!!] YahooSubsystem: Data size too small.  Most likely yahoo captcha has activated.  Try enabling the proxy subsystem.  Retrying (with new proxy if available).");
		yahoo_retries++;
		if(yahoo_retries < 10)
			goto repeat_request;
	}

	printf("\n [+] sr (%u): %s", yahoo_retries, search_request);
	...
	if(!strstr(session->http_session->page->page_data, yahoo_completion_indicator_2))
	{
		session->search_completed_successfully = 1;
	}

	if(strstr(session->http_session->page->page_data, yahoo_captcha_indicator_1))
	{
		printf("\n [!!] Notice! Yahoo captcha has been activated.  Stopping search.");
    ...
    size_t yahoo_blacklist_count = sizeof(yahoo_blacklist) / sizeof(char *);

    // apply html blacklist here to remove servers from anchors and forms set
    HTMLApplyBlacklist(session->last_extraction, (char **) &yahoo_blacklist, yahoo_blacklist_count);
	...
	if(!anchors)
	{
		printf("\n [!!] No anchors.  Completed due to lack of available data (finished).");
